...
private struct WhatsNewSheet: View {
    ...
    private var changes: [(String, String)] {
        [
            (
                language.text("Spielfeld festlegen: Auto-Label erkennt nur, wer auf dem Feld steht", "Set field boundaries: auto-label only detects people standing on the field", "Definir el campo: el marcado automático solo detecta a quien está en el campo", "Définir le terrain : le marquage automatique ne détecte que ceux qui se tiennent sur le terrain"),
                language.text("Neuer Button „Spielfeld festlegen“: Feldmaße eintragen und die vier Eckpunkte im Bild anklicken. Danach berücksichtigt „Automatisch markieren“ bei Spieler/Schiedsrichter/Torwart nur noch Personen, die mit den Füßen tatsächlich auf dem markierten Feld stehen - Zuschauer, Bank und Betreuer werden ignoriert. Bestehende Projekte ohne Markierung funktionieren unverändert weiter.", "New \"Set field boundaries\" button: enter the field's dimensions and click its four corners in the image. Afterwards, \"Auto-label\" for player/referee/goalkeeper only considers people whose feet are actually standing on the marked field - spectators, bench, and staff are ignored. Existing projects without marking keep working exactly as before.", "Nuevo botón «Definir el campo»: introduce las medidas del campo y haz clic en sus cuatro esquinas en la imagen. A partir de entonces, «Marcado automático» para jugador/árbitro/portero solo considera a personas cuyos pies estén realmente sobre el campo marcado; se ignoran espectadores, banquillo y personal. Los proyectos existentes sin marcar siguen funcionando igual que antes.", "Nouveau bouton « Définir le terrain » : saisissez les dimensions du terrain et cliquez sur ses quatre coins dans l’image. Ensuite, « Marquage automatique » pour joueur/arbitre/gardien ne prend en compte que les personnes ayant réellement les pieds sur le terrain marqué - spectateurs, banc et staff sont ignorés. Les projets existants sans marquage continuent de fonctionner comme avant.")
            )
        ]
    }
