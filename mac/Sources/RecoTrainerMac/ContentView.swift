...
private struct WhatsNewSheet: View {
    ...
    private var changes: [(String, String)] {
        [
            (
                language.text("Auto-Label verschwendet keine Zeit mehr auf unbekannte Klassen", "Auto-label no longer wastes time on classes the model doesn't know", "El marcado automático ya no pierde tiempo con clases que el modelo no conoce", "Le marquage automatique ne perd plus de temps sur des classes inconnues du modèle"),
                language.text("Ein aktiviertes Modell, das z. B. nur mit „Ball“ trainiert wurde, ließ das automatische Markieren trotzdem eine volle Erkennung für „Schiedsrichter“ oder „Spieler“ durchlaufen und meldete am Ende nur ein pauschales „0 Boxen“. Reco Trainer prüft jetzt die tatsächlichen Klassen des aktivierten Modells und überspringt automatisch, was es nachweislich nicht kennt - mit einer klaren Erklärung je Klasse.", "An activated model trained on e.g. only \"ball\" still had a full detection pass run for \"referee\" or \"player\", ending in a single blanket \"0 boxes\". Reco Trainer now checks the activated model's actual classes and automatically skips whatever it provably doesn't know - with a clear explanation per class.", "Un modelo activado entrenado, por ejemplo, solo con «balón» aún ejecutaba una detección completa para «árbitro» o «jugador», terminando en un genérico «0 cuadros». Reco Trainer ahora comprueba las clases reales del modelo activado y omite automáticamente lo que demostrablemente no conoce, con una explicación clara por clase.", "Un modèle activé entraîné par exemple uniquement sur « ballon » exécutait quand même une détection complète pour « arbitre » ou « joueur », se terminant par un simple « 0 boîte ». Reco Trainer vérifie désormais les classes réelles du modèle activé et ignore automatiquement ce qu’il ne connaît manifestement pas, avec une explication claire par classe.")
            )
        ]
    }
