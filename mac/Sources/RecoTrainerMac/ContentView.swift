...
private struct WhatsNewSheet: View {
    ...
    private var changes: [(String, String)] {
        [
            (
                language.text("Hinweis auf neue Versionen", "Notice for new versions", "Aviso de nuevas versiones", "Avis de nouvelles versions"),
                language.text("Reco Trainer prüft jetzt beim Start einmal, ob auf GitHub eine neuere Version veröffentlicht wurde, und zeigt bei Bedarf einen Hinweis mit Download-Link. Dabei werden ausschließlich die öffentlichen GitHub-Releases abgefragt - keine Projektdaten verlassen den Rechner.", "Reco Trainer now checks once at launch whether a newer version has been published on GitHub, showing a banner with a download link if so. Only GitHub's public releases are queried - no project data leaves the computer.", "Reco Trainer ahora comprueba una vez al iniciar si se ha publicado una versión más reciente en GitHub y muestra un aviso con un enlace de descarga si corresponde. Solo se consultan los lanzamientos públicos de GitHub; ningún dato del proyecto sale del ordenador.", "Reco Trainer vérifie désormais une fois au démarrage si une version plus récente a été publiée sur GitHub et affiche une bannière avec un lien de téléchargement le cas échéant. Seules les versions publiques de GitHub sont interrogées - aucune donnée de projet ne quitte l’ordinateur.")
            ),
            (
                language.text("OpenCV-Verfeinerung jetzt auch für von Hand gezeichnete Boxen", "OpenCV refinement now also covers hand-drawn boxes", "El refinamiento con OpenCV ahora también cubre cuadros dibujados a mano", "L’affinement OpenCV couvre désormais aussi les boîtes dessinées à la main"),
                language.text("„Boxen mit OpenCV verfeinern“ (vormals „Ball-Boxen mit OpenCV prüfen“) zieht jetzt auf Wunsch auch manuell gezeichnete Boxen nach, nicht mehr nur automatische Vorschläge. Nur plausible, eng anliegende Anpassungen werden übernommen; die ursprünglichen Koordinaten bleiben je Box gespeichert.", "\"Refine boxes with OpenCV\" (previously \"Review ball boxes with OpenCV\") now also tightens manually drawn boxes on request, not just automatic suggestions. Only plausible, closely-matching adjustments are applied; each box's original coordinates remain stored.", "«Refinar cuadros con OpenCV» (antes «Revisar cuadros de balón con OpenCV») ahora también ajusta cuadros dibujados a mano si se solicita, no solo sugerencias automáticas. Solo se aplican ajustes plausibles y cercanos; las coordenadas originales de cada cuadro permanecen guardadas.", "« Affiner les boîtes avec OpenCV » (auparavant « Vérifier les boîtes de ballon avec OpenCV ») resserre désormais aussi les boîtes dessinées à la main sur demande, pas seulement les suggestions automatiques. Seuls les ajustements plausibles et proches sont appliqués ; les coordonnées d’origine de chaque boîte restent enregistrées.")
            ),
            (
                language.text("„Spieler“ bleibt auswählbar, auch ohne eigens trainierte Klasse", "\"Player\" stays selectable even without a dedicated trained class", "«Jugador» sigue siendo seleccionable incluso sin una clase entrenada específica", "« Joueur » reste sélectionnable même sans classe spécifiquement entraînée"),
                language.text("Auch wenn das aktive Modell z. B. nur „Ball“ gelernt hat, bleibt „Spieler“ beim automatischen Markieren wählbar: Reco Trainer nutzt dafür zusätzlich das allgemeine Basismodell (erkennt Personen generisch, nicht speziell als Spieler). „Schiedsrichter“ & Co. bleiben ausgegraut, da es dafür keine Basismodell-Entsprechung gibt.", "Even when the active model has learned e.g. only \"ball\", \"player\" stays selectable for auto-label: Reco Trainer additionally uses the generic base model for it (detects people generically, not specifically as players). \"Referee\" and similar stay greyed out since there's no base-model equivalent for them.", "Aunque el modelo activo solo haya aprendido, por ejemplo, «balón», «jugador» sigue siendo seleccionable para el marcado automático: Reco Trainer utiliza además el modelo base genérico para ello (detecta personas de forma genérica, no específicamente como jugadores). «Árbitro» y similares siguen en gris, ya que no existe un equivalente en el modelo base.", "Même si le modèle actif n’a appris par exemple que « ballon », « joueur » reste sélectionnable pour le marquage automatique : Reco Trainer utilise en plus le modèle de base générique pour cela (détecte les personnes génériquement, pas spécifiquement comme joueurs). « Arbitre » et similaires restent grisés, faute d’équivalent dans le modèle de base.")
            )
        ]
    }
