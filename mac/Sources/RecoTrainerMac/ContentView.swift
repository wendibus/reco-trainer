...
private struct WhatsNewSheet: View {
    ...
    private var changes: [(String, String)] {
        [
            (
                language.text("Mehrere Trainingsbilder auf einmal entfernen", "Remove several training images at once", "Quitar varias imágenes de entrenamiento a la vez", "Retirer plusieurs images d’entraînement à la fois"),
                language.text("Die Trainingsbilder-Liste unterstützt jetzt ⌘-Klick und Umschalt-Klick für Mehrfachauswahl, mit einem eigenen „Entfernen“-Knopf für die Auswahl. Die Bestätigung beim Entfernen hat außerdem einen „Nicht mehr fragen“-Haken bekommen.", "The training-image list now supports Cmd-click and Shift-click for multi-select, with its own \"Remove\" button for the selection. The removal confirmation also gained a \"Don't ask again\" checkbox.", "La lista de imágenes de entrenamiento ahora admite Cmd+clic y Mayús+clic para selección múltiple, con su propio botón «Quitar» para la selección. La confirmación al quitar también incorpora una casilla «No volver a preguntar».", "La liste des images d’entraînement prend désormais en charge Cmd-clic et Maj-clic pour la sélection multiple, avec son propre bouton « Retirer » pour la sélection. La confirmation de retrait a aussi une case « Ne plus demander ».")
            ),
            (
                language.text("Training auf ausgewählte Klassen beschränken", "Restrict training to selected classes", "Restringir el entrenamiento a clases seleccionadas", "Limiter l’entraînement aux classes sélectionnées"),
                language.text("Neue Klassen-Chips bei „4 · Modell verbessern“ lassen Dich einzelne Klassen von diesem Trainingslauf ausschließen (z. B. nur Ball und Schiedsrichter trainieren). Keine Garantie, dass eine ausgeschlossene Klasse dadurch exakt gleich bleibt - sie wird lediglich nicht in diesem Lauf berücksichtigt.", "New class chips in \"4 · Improve model\" let you exclude individual classes from this training run (e.g. train only ball and referee). No guarantee an excluded class stays exactly unchanged - it's simply not part of this run.", "Los nuevos chips de clase en «4 · Mejorar modelo» permiten excluir clases individuales de este entrenamiento (p. ej. entrenar solo balón y árbitro). No hay garantía de que una clase excluida permanezca exactamente igual; simplemente no forma parte de este entrenamiento.", "De nouvelles puces de classe dans « 4 · Améliorer le modèle » permettent d’exclure certaines classes de cet entraînement (p. ex. n’entraîner que ballon et arbitre). Aucune garantie qu’une classe exclue reste exactement inchangée - elle ne fait simplement pas partie de cet entraînement.")
            )
        ]
    }
