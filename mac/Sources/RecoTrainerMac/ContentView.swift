...
private struct WhatsNewSheet: View {
    ...
    private var changes: [(String, String)] {
        [
            (
                language.text("Modellpakete: Prüfung jetzt bei jeder Nutzung verpflichtend", "Model packages: the check is now mandatory on every use", "Paquetes de modelo: la comprobación ahora es obligatoria en cada uso", "Paquets de modèle : la vérification est désormais obligatoire à chaque utilisation"),
                language.text("Die Code-Sicherheitsprüfung aus der letzten Version konnte übersprungen werden, wenn die ML-Umgebung beim Import noch nicht eingerichtet war - eine Gewichtsdatei landete dann ungeprüft in der Bibliothek. Jetzt läuft dieselbe Prüfung zusätzlich unmittelbar vor jeder tatsächlichen Nutzung einer Gewichtsdatei (Training, automatisches Markieren, Modellvergleich, Export), an der Stelle, an der die ML-Umgebung ohnehin vorhanden sein muss - keine Ausnahme mehr möglich. Zip-Einträge, die als Symlink markiert sind, werden jetzt ebenfalls grundsätzlich abgelehnt.", "The code-safety check from the previous version could be skipped if the ML environment wasn't set up yet at import time - a weights file could then end up in the library unscanned. The same check now additionally runs immediately before every actual use of a weights file (training, auto-labeling, model comparison, export), at the point where the ML environment must already exist anyway - no exception possible anymore. Zip entries marked as symlinks are now also rejected outright.", "La comprobación de seguridad del código de la versión anterior podía omitirse si el entorno de ML aún no estaba configurado al importar, dejando un archivo de pesos sin comprobar en la biblioteca. Ahora la misma comprobación también se ejecuta justo antes de cada uso real de un archivo de pesos (entrenamiento, marcado automático, comparación de modelos, exportación), en el punto donde el entorno de ML ya debe existir de todos modos, sin excepción posible. Las entradas zip marcadas como enlaces simbólicos también se rechazan ahora por completo.", "La vérification de sécurité du code de la version précédente pouvait être ignorée si l’environnement ML n’était pas encore configuré à l’import - un fichier de poids pouvait alors se retrouver non vérifié dans la bibliothèque. La même vérification s’exécute désormais aussi juste avant chaque utilisation réelle d’un fichier de poids (entraînement, marquage automatique, comparaison de modèles, export), à l’endroit où l’environnement ML doit de toute façon déjà exister - plus aucune exception possible. Les entrées zip marquées comme liens symboliques sont désormais aussi rejetées d’office.")
            ),
            (
