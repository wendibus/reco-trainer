...
private struct WhatsNewSheet: View {
    ...
    private var changes: [(String, String)] {
        [
            (
                language.text("Modellverwaltung zeigt die Klassen jedes Modells", "Model library shows each model's classes", "La biblioteca de modelos muestra las clases de cada modelo", "La bibliothèque de modèles affiche les classes de chaque modèle"),
                language.text("Jeder Modelleintrag in der Modellverwaltung zeigt jetzt direkt, welche Klassen das Modell tatsächlich kann (z. B. „Ball, Spieler“) - vorher ließ sich das nur durch manuelles Nachsehen im Modellpaket herausfinden.", "Every model entry in the model library now shows directly which classes that model actually knows (e.g. \"ball, player\") - previously this could only be found by manually inspecting the model package.", "Cada entrada de la biblioteca de modelos ahora muestra directamente qué clases conoce realmente ese modelo (p. ej. «balón, jugador») - antes solo se podía averiguar revisando manualmente el paquete del modelo.", "Chaque entrée de la bibliothèque de modèles affiche désormais directement les classes que ce modèle connaît réellement (p. ex. « ballon, joueur ») - auparavant, cela ne pouvait être découvert qu’en inspectant manuellement le paquet du modèle.")
            ),
            (
                language.text("Windows: „ML einrichten“ schlug fehl", "Windows: \"Set up ML\" failed", "Windows: «Configurar ML» fallaba", "Windows : « Configurer le ML » échouait"),
                language.text("Auf Windows scheiterte „ML einrichten“ (und jeder folgende lokale ML-Schritt) mit „Die angegebene Datei wurde nicht gefunden“, weil die App intern einen reinen macOS/Linux-Dateipfad für die virtuelle Python-Umgebung erwartet hat. Behoben.", "On Windows, \"Set up ML\" (and every following local ML step) failed with \"the system cannot find the file specified\", because the app internally expected a macOS/Linux-only file path for the Python virtual environment. Fixed.", "En Windows, «Configurar ML» (y cada paso de ML local posterior) fallaba con «no se encuentra el archivo especificado», porque la app esperaba internamente una ruta de archivo exclusiva de macOS/Linux para el entorno virtual de Python. Corregido.", "Sous Windows, « Configurer le ML » (et chaque étape de ML locale suivante) échouait avec « le fichier spécifié est introuvable », car l’application attendait en interne un chemin de fichier propre à macOS/Linux pour l’environnement virtuel Python. Corrigé.")
            )
        ]
    }
