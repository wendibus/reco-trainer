...
type LocalFrame = { id: string; relativePath: string; videoName: string; timestamp: number; width: number; height: number; annotations: LocalAnnotation[]; reviewStatus?: 'candidate' | 'reviewed'; heldOut?: boolean };
...
export default function Home() {
  ...
  const independentCandidates = frames.filter((frame) => frame.reviewStatus === 'candidate' && frame.heldOut);
  ...
  async function startIndependentValidation() {
    try { await post('/api/independent-validation', { model: modelSize, language, threshold: .12, framesPerVideo }); }
    catch (error) { setWorker((current) => current ? { ...current, error: String(error) } : current); }
  }
