...
import BallTrackingPlayer from './BallTrackingPlayer';
...
type LocalFrame = { id: string; relativePath: string; videoName: string; timestamp: number; width: number; height: number; annotations: LocalAnnotation[]; reviewStatus?: 'candidate' | 'reviewed'; heldOut?: boolean; reviewFlags?: string[] };
...
type PerClassBenchmarkMetrics = { groundTruth: number; predictions: number; truePositives: number; falsePositives: number; falseNegatives: number; precision: number; recall: number; f1: number; ap50: number; meanIoU: number };
type BenchmarkMetrics = { qualityScore: number; mAP50: number; precision: number; recall: number; f1: number; meanIoU: number; truePositives: number; falsePositives: number; falseNegatives: number; perClass?: Record<string, PerClassBenchmarkMetrics> };
type BenchmarkReport = { createdAt?: string; runID?: string; datasetID?: string; matchesGroundTruth?: boolean; frameCount: number; annotationCount: number; modelCount: number; successfulModelCount: number; threshold: number; device: string; rankingMethod: string; classes: string[]; results: Array<{ rank?: number | null; packageID: string; modelSize?: string; status: string; metrics?: BenchmarkMetrics; meanLatencyMs?: number; error?: string }> };
type WorkerStatus = {
...
export default function Home() {
  ...
  // Separate from `category` (single-select, used for hand-drawing a box):
  // this drives the multi-select "Auto-label" checkboxes so one run can add
  // several classes at once instead of one category per run.
  const [autoLabelCategories, setAutoLabelCategories] = useState<string[]>(['ball']);
  ...
  const [useEnsembleDisagreement, setUseEnsembleDisagreement] = useState(false);
  const [trainFreshStart, setTrainFreshStart] = useState(false);
  const [workspaceView, setWorkspaceView] = useState<'training' | 'benchmark' | 'simulation'>('training');
  ...
  const isFlaggedForReview = (frame: LocalFrame) => (frame.reviewFlags?.length ?? 0) > 0;
  const candidates = frames.filter((frame) => frame.reviewStatus === 'candidate' && !frame.heldOut).sort((left, right) => {
    const flagged = Number(isFlaggedForReview(right)) - Number(isFlaggedForReview(left));
  ...
  const independentCandidates = frames.filter((frame) => frame.reviewStatus === 'candidate' && frame.heldOut).sort((left, right) => Number(isFlaggedForReview(right)) - Number(isFlaggedForReview(left)));
  ...
  const reviewFlagLabel = (flag: string) => ({ 'ensemble-disagreement': t.flagDisagreement, 'temporal-outlier': t.flagTemporalOutlier }[flag] ?? flag);
  ...
  useEffect(() => {
    ...
    setUseCuda(window.localStorage.getItem('reco-use-cuda-v1') !== 'no');
  }, []);
  ...
  async function commitAnnotations(annotations: LocalAnnotation[], remember = true) {
    ...
    if (remember) { setHistory((items) => [...items, before].slice(-10)); setFuture([]); }
  ...
  async function chooseFolder() {
    try {
      ...
      if (result.status?.sport) { setSport(result.status.sport as Sport); setCategory(result.status.sport === 'hockey' ? 'puck' : 'ball'); setAutoLabelCategories([result.status.sport === 'hockey' ? 'puck' : 'ball']); }
  ...
  async function runML(path: string, extra: Record<string, unknown> = {}) { try { await post(path, { model: modelSize, epochs: 20, language, category, threshold, ...extra }); } catch (error) { setWorker((current) => current ? { ...current, error: String(error) } : current); } }
  async function createPackage() {
    const promptText = language === 'de' ? 'Wie soll das Modellpaket heißen?' : language === 'es' ? '¿Cómo debe llamarse el paquete del modelo?' : language === 'fr' ? 'Comment nommer le paquet du modèle ?' : 'What should the model package be called?';
    const suggestion = `${sportName} · ${modelSize === 'nano' ? 'Nano' : 'Small'}`;
    const name = window.prompt(promptText, suggestion)?.trim();
    if (name) await runML('/api/package-model', { name });
  }
  async function startActiveLearning() {
    try { await post('/api/active-learning', { model: modelSize, language, category, threshold: .12, framesPerVideo, compareModels: useEnsembleDisagreement }); }
    catch (error) { setWorker((current) => current ? { ...current, error: String(error) } : current); }
  }
  ...
  async function startIndependentValidation() {
    try { await post('/api/independent-validation', { model: modelSize, language, threshold: .12, framesPerVideo, compareModels: useEnsembleDisagreement }); }
    catch (error) { setWorker((current) => current ? { ...current, error: String(error) } : current); }
  }
  async function confirmIndependentCandidate() {
    if (!selectedFrame || selectedFrame.reviewStatus !== 'candidate') return;
    const next = independentCandidates.find((frame) => frame.id !== selectedFrame.id);
    try {
      await post('/api/review-candidate', { frameId: selectedFrame.id, decision: 'confirm' });
      setSelectedFrameID(next?.id ?? selectedFrame.id);
      setSelectedAnnotationID(null); setHistory([]); setFuture([]);
    } catch (error) { setWorker((current) => current ? { ...current, error: String(error) } : current); }
  }
  ...
  async function undo() { if (!selectedFrame || !history.length) return; const previous = history[history.length - 1]; setHistory((items) => items.slice(0, -1)); setFuture((items) => [...items, cloneAnnotations(selectedFrame.annotations)].slice(-10)); await commitAnnotations(previous, false); }
  async function redo() { if (!selectedFrame || !future.length) return; const next = future[future.length - 1]; setFuture((items) => items.slice(0, -1)); setHistory((items) => [...items, cloneAnnotations(selectedFrame.annotations)].slice(-10)); await commitAnnotations(next, false); }
  ...
  const simulationLabel = language === 'de' ? 'Balltracking simulieren' : language === 'es' ? 'Simular seguimiento del balón' : language === 'fr' ? 'Simuler le suivi du ballon' : 'Simulate ball tracking';
  ...
  return <main className="preview-page">
    <header className="preview-header">
      ...
      <div className="header-actions"><button type="button" className={workspaceView === 'benchmark' ? 'walkthrough-launch active-view' : 'walkthrough-launch'} onClick={() => setWorkspaceView((view) => view === 'benchmark' ? 'training' : 'benchmark')}>{workspaceView === 'benchmark' ? `← ${trainingLabel}` : `◇ ${benchmarkLabel}`}</button><button type="button" className={workspaceView === 'simulation' ? 'walkthrough-launch active-view' : 'walkthrough-launch'} onClick={() => setWorkspaceView((view) => view === 'simulation' ? 'training' : 'simulation')}>{workspaceView === 'simulation' ? `← ${trainingLabel}` : `◎ ${simulationLabel}`}</button><button type="button" className="walkthrough-launch" onClick={() => { setWalkthroughStep(0); setWalkthroughOpen(true); }}>? {t.help}</button><div className="local-note"><span>●</span>{t.note}</div></div>
    </header>
    ...
    <section className="preview-stage" aria-label={`${platform} Vorschau`}><div className={`app-window platform-${platform}`}><div className="app-body">
      <aside className="app-sidebar">
        ...
        <section className="side-section active-learning-section"><label>{at.independentTitle}</label><button type="button" className="secondary-button" onClick={() => void startIndependentValidation()} disabled={!framesReady || isWorking}>＋ {at.independentStart}</button><small>{at.independentDetail}</small>{independentCandidates.length > 0 && <strong>{independentCandidates.length} {at.independentQueue}</strong>}</section>
        <section className="side-section frames-section"><label>3 · {t.images}</label><div className="frame-list">{framesReady ? frames.map((frame) => <button type="button" className={`${selectedFrame?.id === frame.id ? 'frame active' : 'frame'} ${frame.reviewStatus === 'candidate' ? 'candidate' : ''}`} key={frame.id} onClick={() => chooseFrame(frame.id)}><img className="frame-thumb" src={`${api}/api/frame?id=${encodeURIComponent(frame.id)}`} alt="" loading="lazy" decoding="async" />{isFlaggedForReview(frame) && <span className="frame-flag" title={(frame.reviewFlags ?? []).map(reviewFlagLabel).join(', ')}>⚠️</span>}<span>{frame.videoName}<small>{String(Math.floor(frame.timestamp / 60)).padStart(2, '0')}:{String(Math.floor(frame.timestamp % 60)).padStart(2, '0')} · {frame.annotations.length}{frame.reviewStatus === 'candidate' ? ` · ${frame.heldOut ? at.independentQueue : at.queue}` : ''}</small></span></button>) : <p className="empty-frames">{worker?.message || t.emptyFrames}</p>}</div></section>
        <div className="privacy-lock"><span>●</span> {t.privacy}</div>
      </aside>
      {workspaceView === 'benchmark' ? <BenchmarkPanel language={language} busy={isWorking} operation={worker?.operation} progress={worker?.progress ?? 0} message={worker?.message} error={worker?.error} log={worker?.log} frameCount={stats.frameCount} automaticCount={stats.automaticCount} sport={worker?.sport ?? sport} models={worker?.modelLibrary?.packages ?? []} benchmark={worker?.benchmark} post={post} /> : workspaceView === 'simulation' ? <BallTrackingPlayer language={language} api={api} busy={isWorking} operation={worker?.operation} progress={worker?.progress ?? 0} message={worker?.message} error={worker?.error} simulation={worker?.simulation} post={post} /> : <section className={`workspace ${framesReady ? '' : 'empty-workspace'}`}>
        ...
        {selectedFrame?.reviewStatus === 'candidate' && selectedFrame.heldOut && <section className="candidate-review independent-review"><div><strong>{at.independentReview}: {independentCandidates.indexOf(selectedFrame) + 1} / {independentCandidates.length}</strong><small>{at.independentHint}</small></div><button type="button" className="candidate-yes" onClick={() => void confirmIndependentCandidate()}>✓ {at.independentConfirm}</button><button type="button" onClick={() => void skipCandidate()}>{at.skip}</button></section>}
        {selectedFrame?.reviewStatus === 'candidate' && !selectedFrame.heldOut && <section className="candidate-review"><div><strong>{at.queue}: {candidates.indexOf(selectedFrame) + 1} / {candidates.length}</strong><small>{at.needsBox} {at.hint}</small></div><button type="button" className="candidate-yes" onClick={() => void reviewCandidate('ball')}>✓ {at.ball}</button><button type="button" className="candidate-no" onClick={() => void reviewCandidate('no-ball')}>× {at.noBall}</button><button type="button" onClick={() => setMode('draw')}>⌖ {at.correct}</button><button type="button" onClick={() => void skipCandidate()}>{at.skip}</button></section>}
        {framesReady && <section className="training-card">
          ...
          <div className="training-controls"><label>{t.confidence} <strong>{Math.round(threshold * 100)}%</strong><input type="range" min="0.05" max="0.90" step="0.05" value={threshold} onChange={(event) => setThreshold(Number(event.target.value))} /></label><div className="auto-label-categories">{categories.map((item) => <label key={item} className="auto-label-category-toggle"><input type="checkbox" checked={autoLabelCategories.includes(item)} onChange={() => setAutoLabelCategories((current) => current.includes(item) ? current.filter((entry) => entry !== item) : [...current, item])} />{categoryName(item)}</label>)}</div><label className="cuda-toggle" title={t.ensembleToggleHint}><input type="checkbox" checked={useEnsembleDisagreement} onChange={(event) => setUseEnsembleDisagreement(event.target.checked)} />{t.ensembleToggle}</label><label className="cuda-toggle" title={t.freshStartHint}><input type="checkbox" checked={trainFreshStart} onChange={(event) => setTrainFreshStart(event.target.checked)} />{t.freshStart}</label><div className="training-actions"><label className="cuda-toggle" title={t.cudaToggleHint}><input type="checkbox" checked={useCuda} onChange={(event) => { setUseCuda(event.target.checked); window.localStorage.setItem('reco-use-cuda-v1', event.target.checked ? 'yes' : 'no'); }} />{t.cudaToggle}</label><button type="button" className="secondary-button" onClick={() => void runML('/api/setup', { useCuda })} disabled={isWorking}>{t.setup}</button><button type="button" className="secondary-button" onClick={() => void runML('/api/autolabel', { categories: autoLabelCategories, compareModels: useEnsembleDisagreement })} disabled={isWorking || !autoLabelCategories.length}>{t.auto}: {autoLabelCategories.map(categoryName).join(', ')}</button><button type="button" className="primary-button" onClick={() => void runML('/api/train', { freshStart: trainFreshStart })} disabled={isWorking}>{t.train}</button><button type="button" className="secondary-button" onClick={() => void runML('/api/export-coreml')} disabled={isWorking}>{t.export}</button><button type="button" className="secondary-button" onClick={() => void createPackage()} disabled={isWorking}>{t.share}</button><button type="button" className="secondary-button" onClick={() => void runML('/api/import-model')} disabled={isWorking || !folderName}>{t.importModel}</button></div></div>
