...
rm -f "$OUTPUT_DIR/Reco Trainer Windows 0.8.zip"
(cd "$STAGE_DIR" && zip -q -r "$OUTPUT_DIR/Reco Trainer Windows 0.8.zip" "Reco Trainer Windows")
...
xattr -cr "$LINUX_DIR"
rm -f "$OUTPUT_DIR/Reco Trainer Linux 0.8.zip"
(cd "$STAGE_DIR" && zip -q -r "$OUTPUT_DIR/Reco Trainer Linux 0.8.zip" "Reco Trainer Linux")
...
xattr -cr "$DOCKER_DIR"
rm -f "$OUTPUT_DIR/Reco Trainer Docker 0.8.zip"
(cd "$STAGE_DIR" && zip -q -r "$OUTPUT_DIR/Reco Trainer Docker 0.8.zip" "Reco Trainer Docker")

printf '%s\n' \
  "$OUTPUT_DIR/Reco Trainer Windows 0.8.zip" \
  "$OUTPUT_DIR/Reco Trainer Linux 0.8.zip" \
  "$OUTPUT_DIR/Reco Trainer Docker 0.8.zip"
